...
def test_format_src_trivial():
    after, _ = pyupgrade_docs.format_str("", ARGS)
    assert after == ""


def test_format_src_markdown_simple():
    before = "```python\nprint 1,2,3\n```\n"
    after, _ = pyupgrade_docs.format_str(before, ARGS)
    assert after == ("```python\nprint(1, 2, 3)\n```\n")
...
def test_integration_ok(tmpdir, capsys):
    f = tmpdir.join("f.md")
    f.write("```python\n" "f(1, 2, 3)\n" "```\n")
    assert not pyupgrade_docs.main((str(f),))
...
def test_integration_modifies(tmpdir, capsys):
    f = tmpdir.join("f.md")
    f.write("```python\n" "f(1,2,3)\n" "```\n")
    assert pyupgrade_docs.main((str(f),))
...
def test_integration_line_length(tmpdir):
    ...
    assert not pyupgrade_docs.main((str(f), "--line-length=80"))
    assert pyupgrade_docs.main((str(f), "--line-length=50"))
    assert f.read() == (
        "```python\n"
        "foo(\n"
...
def test_integration_py36(tmpdir):
    ...
    assert not pyupgrade_docs.main((str(f),))
    assert pyupgrade_docs.main((str(f), "--target-version=py36"))
    assert f.read() == (
        "```python\n"
        "def very_very_long_function_name(\n"
        "    very_very_very_very_very_very,\n"
...
def test_integration_filename_last(tmpdir):
    ...
    assert not pyupgrade_docs.main((str(f),))
    assert pyupgrade_docs.main(("--target-version", "py36", str(f)))
    assert f.read() == (
        "```python\n"
        "def very_very_long_function_name(\n"
        "    very_very_very_very_very_very,\n"
...
def test_integration_multiple_target_version(tmpdir):
    ...
    assert not pyupgrade_docs.main((str(f),))
    assert not pyupgrade_docs.main(
...
def test_integration_skip_string_normalization(tmpdir):
    f = tmpdir.join("f.md")
    f.write("```python\n" "f('hi')\n" "```\n")
    assert not pyupgrade_docs.main((str(f), "--skip-string-normalization"))
    assert f.read() == ("```python\n" "f('hi')\n" "```\n")
...
def test_integration_ignored_syntax_error(tmpdir, capsys):
    f = tmpdir.join("f.md")
    f.write("```python\n" "f( )\n" "```\n" "\n" "```python\n" "f(\n" "```\n")
    assert pyupgrade_docs.main((str(f), "--skip-errors"))
    out, _ = capsys.readouterr()
    assert f.read() == ("```python\n" "f()\n" "```\n" "\n" "```python\n" "f(\n" "```\n")
